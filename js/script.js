         ...
         success: function (resp) {
            if (!!resp.status) {
               if (resp.status == 'success') {
                  var elm = $('<div>')
                  elm.hide()
                  elm.addClass('alert alert-primary alert_msg')
                  elm.text("Data successfulle saved");
                  ...
                  var elm = $('<div>')
                  elm.hide()
                  elm.addClass('alert alert-danger alert_msg form-group')
                  elm.text(resp.msg);
                  $('#new-author-frm').append(elm)
                  elm.show('slow')
               } else {
                  alert("An error occured. Please chech the source code and try again")
               }
         ...
         success: function (resp) {
            if (!!resp.status) {
               if (resp.status == 'success') {
                  ...
                  elm.hide()
                  elm.addClass('alert alert-primary alert_msg')
                  elm.text("Data successfulle updated");
